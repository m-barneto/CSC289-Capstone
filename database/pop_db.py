...
def populate():
    ...
    try:
        ...
        NUM_ENTRIES = 9
        user_ids = [i for i in range(1, NUM_ENTRIES)]
        ...
        dummy_data = zip(user_ids, usernames, passwords, emails, phone_numbers, degrees, semesters)
        ...
        with sqlite3.connect('storage.db') as conn:
            sqlite_insert = '''
                ...
                VALUES(?,?,?,?,?,?,?)
            '''
            conn.execute(sqlite_insert, (0, 'mbarneto', '1234', 'email@gmail.com', '9198675309', 'programming', 'SPR 2023'))
            for item in dummy_data:
                conn.execute(sqlite_insert, item)
            conn.commit()
